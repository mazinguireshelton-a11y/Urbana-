...
# ========== NOVA ROTA: AUTH SESSION PARA RECEBER TOKENS DO FRONTEND ==========
@app.route("/auth/session", methods=["POST"])
def auth_session():
    """Recebe os tokens enviados pelo frontend (capturados do fragmento da URL)"""
    try:
        ...
        # Buscar usuário no Supabase usando o token
        ...
        if user_response.status_code != 200:
            print(f"❌ Token inválido: {user_response.status_code}")
        ...
        ensure_user_profile(user_id, email, name, 'Google')
        ...
        print(f"🎉 Login via Google bem sucedido: {name} ({email})")
        
        return jsonify({"success": True})
        
    except Exception as e:
        print(f"❌ Erro em auth_session: {e}")
        import traceback
        traceback.print_exc()
        return jsonify({"success": False, "error": str(e)}), 500


# ========== ROTA DE CALLBACK MODIFICADA - Renderiza HTML que captura fragmento ==========
@app.route("/auth/callback")
def auth_callback():
    """Callback do Google - Renderiza página HTML que captura o token do fragmento da URL"""
    # Verificar se houve erro na URL
    error = request.args.get('error')
    if error:
        flash(f'Erro do Google: {error}', 'danger')
        return redirect(url_for('login'))
    
    # Renderiza página HTML que vai capturar o token do fragmento
    return render_template("oauth_callback.html")


# ========== ROTA DE CALLBACK LEGACY (MANTIDA PARA COMPATIBILIDADE) ==========
@app.route("/auth/callback-legacy")
def auth_callback_legacy():
    """Callback - Versão Supabase Native (mantida para compatibilidade, mas não funciona com fragmento)"""
    print("=" * 60)
    print("🔄 CALLBACK GOOGLE (LEGACY)")
